...
const CreatePost = () => {
  ...
  const submitTheBlog = async (e) => {
    e.preventDefault();
    console.log( headline, blog, photo, email, selectedTopic,subParagraph1,subParagraph2 );
    try {
  ...
  useEffect(() => {
    ...
    if (isCreateBlogOpen) {
      ...
      gsap.to(parentDivRef.current, { autoAlpha: 0, duration: animationDuration });
    }
  }, [isCreateBlogOpen]);

  return (
    <>
   { isCreateBlogOpen ? <div ref={parentDivRef}  className={`max-w-4xl space-y-4 p-2 mx-auto`} >
